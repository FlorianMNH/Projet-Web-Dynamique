...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['date_heure'])) {
    ...
    if ($conn->query($query_rdv) === TRUE) {
        ...
        if ($day_time_slot) {
            $update_query = "UPDATE AgentImmobilier SET $day_time_slot='Non disponible' WHERE identifiant_agent=$id_agent";
...
<div class="agent-details">
    <h2>Agent: <?php echo $agent['prenom'] . " " . $agent['nom']; ?></h2>
    <p>Email: <?php echo $agent['email']; ?></p>
    <p>Téléphone: <?php echo $agent['telephone']; ?></p>
    <h3>Disponibilité de l'agent</h3>
    <form id="appointment-form" method="POST" action="prendre_rdv.php?id_agent=<?php echo $id_agent; ?>">
